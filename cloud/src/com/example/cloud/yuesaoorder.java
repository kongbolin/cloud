...
public class yuesaoorder extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.yuesaoorder);
		...
		Log.d("yuesaoorder", orderid);
		Log.d("yuesaoorder", userid);
				...
				(new Thread() {
					...
					public void run() {
						...
						try {
							...
							HttpPost httppost = new HttpPost("http://120.27.45.56/deleteyuesaoorder.php");
		...
		(new Thread() {
			...
			public void run() {
				...
				try {
					...
					nameValuePairs.add(new BasicNameValuePair("orderid", orderid));
					HttpClient httpclient = new DefaultHttpClient();
					HttpPost httppost = new HttpPost("http://120.27.45.56/selectyuesaoorder.php");
					httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs, "UTF-8"));
					HttpResponse response = httpclient.execute(httppost);
					HttpEntity entity = response.getEntity();
					is = entity.getContent();

...
public class baomudetai extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.baomudetail);
		...
		Log.d("baomudetail", orderid);
		Log.d("baomudetail", userid);
				...
				(new Thread() {
					...
					public void run() {
						...
						try {
							...
							HttpPost httppost = new HttpPost("http://120.27.45.56/deletebaomuorder.php");
		...
		(new Thread() {
			...
			public void run() {
				...
				try {
					...
					nameValuePairs.add(new BasicNameValuePair("orderid", orderid));
					HttpClient httpclient = new DefaultHttpClient();
					HttpPost httppost = new HttpPost("http://120.27.45.56/selectbaomuorder.php");
					httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs, "UTF-8"));
					HttpResponse response = httpclient.execute(httppost);
					HttpEntity entity = response.getEntity();
					is = entity.getContent();

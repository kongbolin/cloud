...
public class yuerdeta extends Activity {
	...
	// private TextView text_yuer_order_contentscreen;
	// private TextView text_yuer_order_squarescreen;
	// private TextView text_yuer_order_tastescreen;
	// private TextView text_yuer_order_frequencyscreen;
	// private TextView text_yuer_order_datestartscreen;
	// private TextView text_yuer_order_dateendscreen;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.yuerdetail);
		...
		Log.d("yuerdetail", orderid);
		Log.d("yuerdetail", userid);
				...
				(new Thread() {
					...
					public void run() {
						...
						try {
							...
							HttpPost httppost = new HttpPost("http://120.27.45.56/deleteyuerorder.php");
		...
		(new Thread() {
			...
			public void run() {
				...
				try {
					...
					nameValuePairs.add(new BasicNameValuePair("orderid", orderid));
					HttpClient httpclient = new DefaultHttpClient();
					HttpPost httppost = new HttpPost("http://120.27.45.56/selectyuerorder.php");
					httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs, "UTF-8"));
					HttpResponse response = httpclient.execute(httppost);
					HttpEntity entity = response.getEntity();
					is = entity.getContent();
